...
辅助信息
 */
@property(nonatomic, retain, readwrite) NSString *lastGesturePwdStr;//最后一次手势密码
@end
...
- (void)viewDidLoad
{
    ...
    switch (_drawPatternCtlType) {
        case kDrawPatternCtlTypeGesturePwdSetup:    //登录成功后 --- 设置 手势密码
        {
            ...
            [self showCustomNavigationMiddleTitle:@"新的手势密码"];
        }break;
        ...
        case kDrawPatternCtlTypeGesturePwdModify:   //设置页面的 --- 修改 手势密码
        {
            ...
            [self showCustomNavigationMiddleTitle:@"修改手势密码"];
        }break;
...
- (void)createInternalViews
{
    ...
    [_gestureInternalViews.btnForgetGesturePwd addTarget:self
                                                  action:@selector(onBtnForgetGesturePwdClicked:)
                                        forControlEvents:(UIControlEventTouchUpInside)];
    
    [_gestureInternalViews.btnTouchIdUnlock    addTarget:self
                                                  action:@selector(onBtnTouchIdUnlockClicked:)
                                        forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)viewWillLayoutSubviews
{
    ...
    //配置公共的frame -------------------------------- begin
    {
        ...
        _v.frame = CGRectMake(xV,yV, wV,hV);
    ...
    switch (_drawPatternCtlType) {
        case kDrawPatternCtlTypeGesturePwdSetup:    //登录成功后 --- 设置 手势密码
        {
            if (isPhone)
            {
                ...
                _gestureInternalViews.headImageView.frame = CGRectMake(self.view.frame.size.width/2-wImage/2, yImage, wImage, wImage);
                ...
                float hLabel = 20;
                _gestureInternalViews.headLabel.frame = CGRectMake(0, 0, 300, 20);
        ...
        case kDrawPatternCtlTypeGesturePwdVerify:   //已设过手势 --- 验证 手势密码
        {
            if (isPhone)
            {
                ...
                float hLabel = 20;
                _gestureInternalViews.headLabel.frame = CGRectMake(0, 0, 300, hLabel);
                _gestureInternalViews.headLabel.center = CGPointMake(self.view.center.x,
    ...
    switch (_drawPatternCtlType) {
        ...
        case kDrawPatternCtlTypeGesturePwdVerify:   //已设过手势 --- 验证 手势密码
        {
            UIImage * icon = [UIImage imageNamed:@"v2_my_avatar"];
            if (!icon) {icon = [UIImage imageNamed:@"v2_my_avatar"];}
...
- (void)setCurrentAccountStr:(NSString *)currentAccountStr
{
    ...
    {
//        NSObject* savedObject = _currentAccountStr;
    }
    /*
     备份 账号
     */
...
- (void)autoDismissLabelShowText:(NSString *)text
{
    ...
    __block DrawPatternLockViewController * __self = self;
